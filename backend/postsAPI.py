...
class Post():#se crea la clase post
    def __init__(self, codigo, titulo, fecha, descripcion, autor): #definimos sus atributos
        self.codigo = codigo
        self.titulo = titulo
...
class Post_endpoint(Resource): #se crea una clase donde se definiran los endpoint
    ...
    def getAll():# endpoint GET obtiene los post y los envia como objetos en un array "lista_post"
        ...
        for fila in query:
            autor = Autor(fila['nombre'], fila['autor_dni'],fila['fecha_nac'])
            post = Post(fila['id'],fila['titulo'], fila['fecha'], fila['descripcion'], autor.__dict__)
    ...
    def put(self): #endpoint PUT editar un post ya existente
        ...
        query = conn.execute("select count(*) from autores where dni = {}".format(autor['dni']))# se ejecuta la conexión a base de datos,y se realiza la consulta(queremos saber la cant, de autores con el mismo dni)
        resultado = query.cursor.fetchone()[0] #se guaraga el resultado obtenido en la consulta
        if resultado ==  0:# si el resultado es igual a cero se crea un nuevo autor
            conn.execute("insert into autores values('{0}','{1}','{2}')".format(autor['dni'], autor['nombre'], autor['fecha_nac']))#se ejecuta la conexión, se añade datos a la tabla creando el autor

        query = conn.execute("update posts set titulo = '{0}', descripcion = '{1}', autor_dni = '{2}' where id = '{3}'".format(titulo, descripcion, autor['dni'], codigo)) #se ejecuta la conexión a base de datos y se módifican los datos del post
        return {'status': 'Se a modificado el Post'}

    @app.route('/api/posts/<int:codigo>', methods=['GET'])
    def getById(codigo):
        conn = db_connect.connect()
        query = conn.execute("select p.id, p.titulo, p.fecha, p.descripcion, a.nombre, a.dni, a.fecha_nac from posts p join autores a on p.autor_dni = a.dni where id = '{}'".format(codigo))
        fila = query.cursor.fetchall()[0]
        autor = Autor(fila[4], fila[5],fila[6])
        post = Post(fila[0], fila[1], fila[2], fila[3], autor.__dict__)
...
class Autor_endpoint(Resource):
    def get(dni):
        ...
        lista_autores= []
        for fila in query:
            autor = Autor(fila['nombre'],fila['dni'], fila['fecha_nac'])
